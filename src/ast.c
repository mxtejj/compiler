...
function TypeSpec *
type_spec_alloc(Parser *p, TypeSpecKind kind) {
  TypeSpec *t = push_struct(p->arena, TypeSpec);
...
function TypeSpec *
type_spec_proc(Parser *p, DeclArray params, TypeSpec *ret, Stmt *body) {
  TypeSpec *t = type_spec_alloc(p, TypeSpecKind_Proc);
...
function TypeSpec *
type_spec_enum(Parser *p, EnumMemberArray members) {
  TypeSpec *t = type_spec_alloc(p, TypeSpecKind_Enum);
  t->enum_members = members;
  return t;
}

function TypeSpec *
type_spec_aggr(Parser *p, TypeSpecKind kind, AggrFieldArray fields) {
  assert(kind == TypeSpecKind_Struct || kind == TypeSpecKind_Union);
  TypeSpec *t = type_spec_alloc(p, kind);
...
function Decl *
decl_alloc(Parser *p, String8 name, DeclKind kind) {
...
function Decl *
decl_var(Parser *p, String8 name, TypeSpec *type_hint, Expr *init_expr) {
...
function Decl *
decl_const(Parser *p, String8 name, TypeSpec *type_hint, Expr *init_expr, TypeSpec *init_type) {
...
function Expr *
expr_alloc(Parser *p, ExprKind kind) {
...
function Expr *
expr_cast(Parser *p, TypeSpec *type, Expr *e) {
...
function Expr *
expr_call(Parser *p, Expr *e, ExprArray args) {
...
function Expr *
expr_compound(Parser *p, TypeSpec *type, CompoundFieldArray args) {
...
function Expr *
expr_size_of_type(Parser *p, TypeSpec *type) {
  Expr *expr = expr_alloc(p, ExprKind_SizeOf);
...
//- Statements
function Stmt *
stmt_alloc(Parser *p, StmtKind kind) {
...
function Stmt *
stmt_block(Parser *p, StmtArray stmts) {
...
function Stmt *
stmt_switch(Parser *p, Expr *expr, SwitchCaseArray cases) {
...
function void
print_type(Arena *arena, String8List *list, int *indent, TypeSpec *t) {
  ...
  case TypeSpecKind_Enum:
    ...
    for each_index(i, t->enum_members.count) {
      EnumMember it = t->enum_members.v[i];
      print_ln(arena, list, indent);
      str8_list_pushf(arena, list, "(%.*s ", str8_varg(it.name));
      if (it.value) {
        //str8_list_pushf(arena, list, "(%.*s ", str8_varg(it.name));
  ...
  case TypeSpecKind_Union:
    ...
    for each_index(i, t->aggr_fields.count) {
      AggrField it = t->aggr_fields.v[i];
...
internal bool
print_is_symbol(TokenKind kind) {
...
print_expr(Arena *arena, String8List *list, int *indent, Expr *e) {
  ...
  case ExprKind_Compound:
    ...
    for each_index(i, e->compound.args.count) {
      CompoundField *arg = e->compound.args.v[i];
      str8_list_pushf(arena, list, " ");
      switch (arg->kind) {
...
print_stmt(Arena *arena, String8List *list, int *indent, Stmt *s) {
  ...
  case StmtKind_Switch:
    ...
    for each_index(i, s->switch0.cases.count) {
      SwitchCase it = s->switch0.cases.v[i];
